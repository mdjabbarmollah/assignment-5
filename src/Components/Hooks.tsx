...
const jsonfilethekedatar_requestdeya_ooseta_process_kore_return_kora = () => {
  const [etaholoparameter, etaHolofuncitoncountkorarjonno] = useState<CardobjectTypelekhaholo[]>([]);
  const [loadinghocchekina, loadingsetkorarjonno] = useState<boolean>(true);

  useEffect(() => {
    loadingsetkorarjonno(true);

    fetch('/JsondataFromAi.json')
      .then((res) => res.json())
      .then((data) => etaHolofuncitoncountkorarjonno(data))
